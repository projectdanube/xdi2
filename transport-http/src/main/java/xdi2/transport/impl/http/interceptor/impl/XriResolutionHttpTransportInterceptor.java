...
public class XriResolutionHttpTransportInterceptor extends AbstractHttpTransportInterceptor {
	...
	public boolean processGetRequest(HttpTransport httpTransport, HttpRequest request, HttpResponse response, MessagingTargetMount messagingTargetMount) throws Xdi2TransportException, IOException {
		...
		XDIAddress canonicalid = referencePeerRoot.getAddressOfPeerRoot();
	...
	private static XDIAddress[] getProviderIdSynonyms(Graph graph, XDIAddress providerid) {
		...
		XDIAddress[] selfSynonyms = new IteratorArrayMaker<XDIAddress> (new MappingContextNodeAddressIterator(selfPeerRootIncomingReferenceContextNodes)).array(XDIAddress.class);
		for (int i=0; i<selfSynonyms.length; i++) selfSynonyms[i] = XdiPeerRoot.getAddressOfPeerRootArc(selfSynonyms[i].getLastArc());
