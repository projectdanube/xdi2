...
//			PrintStream stream4 = new PrintStream(buffer4);
			...
			/*   DEPRECATED  if ("aparse".equals(parser)) {
				...
				output1 = html(new String(buffer1.toByteArray(), StandardCharsets.UTF_8));
				...
				output2 = new String(buffer2.toByteArray(), StandardCharsets.UTF_8);

				// xml

				output3 = html(new String(buffer3.toByteArray(), StandardCharsets.UTF_8));

				// count

				for (Entry<String, Integer> entry : countEntrySet) stream4.println(entry.getKey() + ": " + entry.getValue());
				output4 = html(new String(buffer4.toByteArray(), StandardCharsets.UTF_8));
				...
				output5 = html(new String(buffer5.toByteArray(), StandardCharsets.UTF_8));

				statistics.displayStats(stream6, "rules");
				output6 = html(new String(buffer6.toByteArray(), StandardCharsets.UTF_8));

				output7 = html(new String(buffer7.toByteArray(), StandardCharsets.UTF_8));
				...
				output8 = html(new String(buffer8.toByteArray(), StandardCharsets.UTF_8));
			} else {
