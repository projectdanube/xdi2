	...
	private void buildPlain(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("Plain XDI get".equals(submit)) {
				...
				message.createGetOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAddress.create("<#email>")));
			} else if ("Plain XDI set".equals(submit)) {

				message.createSetOperation(XDIStatement.fromLiteralComponents(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAddress.create("<#email>"), XDIConstants.XDI_ADD_VALUE), "test@email.com"));
			} else if ("Plain XDI del".equals(submit)) {

				message.createDelOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAddress.create("<#email>")));
			}
	...
	private void buildPublicLinkContract(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("Get public link contract".equals(submit)) {
				...
				publicLinkContract.setPermissionTargetAddress(XDILinkContractConstants.XDI_ADD_GET, publicAddress);
	...
	private void buildGenericLinkContract(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("Get generic link contract".equals(submit)) {
				...
				genericLinkContract.setPermissionTargetAddress(XDILinkContractConstants.XDI_ADD_GET, AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAddress.create("<#email>")));
	...
	private void buildKeyPairs(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("Get key pairs".equals(submit)) {
				...
				message.createGetOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_ENCRYPT_KEYPAIR));
				message.createGetOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_SIG_KEYPAIR));
			} else if ("Generate key pairs".equals(submit)) {

				message.createOperation(GenerateKeyContributor.XDI_ADD_DO_KEYPAIR, XDIStatement.fromComponents(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_ENCRYPT_KEYPAIR), XDIDictionaryConstants.XDI_ADD_IS_TYPE, XDIAddress.create("$rsa$2048")));
				message.createOperation(GenerateKeyContributor.XDI_ADD_DO_KEYPAIR, XDIStatement.fromComponents(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_SIG_KEYPAIR), XDIDictionaryConstants.XDI_ADD_IS_TYPE, XDIAddress.create("$rsa$2048")));
			} else if ("Del key pairs".equals(submit)) {

				message.createDelOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_ENCRYPT_KEYPAIR));
				message.createDelOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_SIG_KEYPAIR));
			}
