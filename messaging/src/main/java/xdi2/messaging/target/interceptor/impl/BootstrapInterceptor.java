...
public class BootstrapInterceptor extends AbstractInterceptor<MessagingTarget> implements Prototype<BootstrapInterceptor> {
	...
	public BootstrapInterceptor instanceFor(PrototypingContext prototypingContext) {
		...
		if (prototypingContext.getOwnerPeerRoot() != null) {
			...
			for (int i=0; i<bootstrapOwnerSynonyms.length; i++) bootstrapOwnerSynonyms[i] = ownerSynonymPeerRoots[i].getAddressOfPeerRoot();
		}
	...
	public void init(MessagingTarget messagingTarget) throws Exception {
		...
		if (this.getBootstrapRootLinkContract()) {
			...
			bootstrapRootLinkContract.setPermissionTargetAddress(XDILinkContractConstants.XDI_ADD_ALL, XDIConstants.XDI_ADD_ROOT);
		...
		if (this.getBootstrapPublicLinkContract()) {
			...
			XDIAddress publicAddress = AddressUtil.concatAddresses(this.getBootstrapOwner(), XDILinkContractConstants.XDI_ADD_PUBLIC);
			bootstrapPublicLinkContract.setPermissionTargetAddress(XDILinkContractConstants.XDI_ADD_GET, publicAddress);
			...
			bootstrapPublicLinkContract.setPermissionTargetStatement(XDILinkContractConstants.XDI_ADD_GET, selfPeerRootRefStatement);
			...
			bootstrapPublicLinkContract.setPermissionTargetStatement(XDILinkContractConstants.XDI_ADD_GET, bootstrapOwnerSynonymsIsRefStatement);
			...
			if (this.getBootstrapOwnerSynonyms() != null) {
				...
				for (XDIAddress bootstrapOwnerSynonym : this.getBootstrapOwnerSynonyms()) {
					...
					bootstrapPublicLinkContract.setPermissionTargetStatement(XDILinkContractConstants.XDI_ADD_GET, bootstrapOwnerSynonymRefStatement);
				}
			}
		}
	...
	private final CopyStrategy replaceSelfVariableCopyStrategy = new CopyStrategy() {
		...
		public ContextNode replaceContextNode(ContextNode contextNode) {
			...
			if (log.isDebugEnabled()) log.debug("Replaced " + contextNodeAddress + " with " + replacedcontextNodeAddress);

			if (contextNodeAddress.equals(replacedcontextNodeAddress)) return super.replaceContextNode(contextNode);

			ContextNode replacedContextNode = GraphUtil.contextNodeFromComponents(replacedcontextNodeAddress);
			CopyUtil.copyContextNodeContents(contextNode, replacedContextNode, null);

			int additionalArcs = replacedcontextNodeAddress.getNumArcs() - contextNodeAddress.getNumArcs();
		...
		public Relation replaceRelation(Relation relation) {
			...
			if (log.isDebugEnabled()) log.debug("Replaced " + targetContextNodeAddress + " with " + replacedtargetContextNodeAddress);

			if (targetContextNodeAddress.equals(replacedtargetContextNodeAddress)) return super.replaceRelation(relation);

			Relation replacedRelation = GraphUtil.relationFromComponents(contextNodeAddress, arc, replacedtargetContextNodeAddress);
