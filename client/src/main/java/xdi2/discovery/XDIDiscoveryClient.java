...
public class XDIDiscoveryClient {
	...
	public XDIDiscoveryResult discoverFromRegistry(XDIAddress query, XDIAddress[] endpointUriTypes) throws Xdi2ClientException {
		...
		if (registryMessageResult != null) {
			...
			registryMessage.createGetOperation(XDIAddress.fromComponent(XdiPeerRoot.createPeerRootArc(query)));
	...
	public XDIDiscoveryResult discoverFromAuthority(String xdiEndpointUri, CloudNumber cloudNumber, XDIAddress[] endpointUriTypes) throws Xdi2ClientException {
		...
		if (authorityMessageResult != null) {
			...
			authorityMessage.createGetOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), XDIAuthenticationConstants.XDI_ADD_MSG_ENCRYPT_KEYPAIR_PUBLIC_KEY));

			if (endpointUriTypes != null) {

				for (XDIAddress endpointUriType : endpointUriTypes) {

					authorityMessage.createGetOperation(AddressUtil.concatAddresses(cloudNumber.getAddress(), endpointUriType, XDIClientConstants.XDI_ADD_AS_URI));
				}
			}
