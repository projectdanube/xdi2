...
final public class Rule_unreserved extends Rule
{
  private Rule_unreserved(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_unreserved parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_unreserved(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
