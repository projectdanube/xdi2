...
final public class Rule_IP_literal extends Rule
{
  private Rule_IP_literal(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_IP_literal parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_IP_literal(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
