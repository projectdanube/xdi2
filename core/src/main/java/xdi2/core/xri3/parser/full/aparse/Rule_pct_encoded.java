...
final public class Rule_pct_encoded extends Rule
{
  private Rule_pct_encoded(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_pct_encoded parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_pct_encoded(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
