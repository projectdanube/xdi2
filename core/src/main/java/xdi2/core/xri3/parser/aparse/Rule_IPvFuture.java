...
final public class Rule_IPvFuture extends Rule
{
  private Rule_IPvFuture(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_IPvFuture parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_IPvFuture(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
