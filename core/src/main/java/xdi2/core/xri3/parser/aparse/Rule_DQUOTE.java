...
final public class Rule_DQUOTE extends Rule
{
  private Rule_DQUOTE(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_DQUOTE parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_DQUOTE(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
