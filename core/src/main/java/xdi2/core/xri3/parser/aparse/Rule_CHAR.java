...
{
  private Rule_CHAR(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_CHAR parse(ParserContext context)
  {
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          {
            rule = Terminal_NumericValue.parse(context, "%x01-7F", "[\\x01-\\x7F]", 1);
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule_CHAR(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;
