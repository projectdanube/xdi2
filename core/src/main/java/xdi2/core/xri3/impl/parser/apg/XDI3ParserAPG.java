...
import xdi2.core.xri3.impl.XDI3SyntaxComponent;
import xdi2.core.xri3.impl.XDI3XRef;
import xdi2.core.xri3.impl.parser.XDI3Parser;
import xdi2.core.xri3.impl.parser.apg.XDI3Grammar.RuleNames;
...
public class XDI3ParserAPG implements XDI3Parser {
	...
	private static Parser parser_subseg = makeParser(XDI3Grammar.RuleNames.XDI_SUBSEG);
	private static Parser parser_xref = makeParser(XDI3Grammar.RuleNames.XDI_XREF);
	...
	private static XDI3SyntaxComponent parse(Parser parser, String input) {
		...
		try {
			...
			ast.enableRuleNode(XDI3Grammar.RuleNames.XDI_SUBSEG.ruleID(), true);
			ast.enableRuleNode(XDI3Grammar.RuleNames.XDI_XREF.ruleID(), true);
			...
			ast.setRuleCallback(XDI3Grammar.RuleNames.XDI_SUBSEG.ruleID(), new MyAstCallback(ast, myAstContext, XDI3Grammar.RuleNames.XDI_SUBSEG));
			ast.setRuleCallback(XDI3Grammar.RuleNames.XDI_XREF.ruleID(), new MyAstCallback(ast, myAstContext, XDI3Grammar.RuleNames.XDI_XREF));
	...
	private static class MyAstCallback extends AstCallback {
		...
		public void postBranch(int offset, int length) {
			...
			if (this.ruleName.equals(RuleNames.XDI_SEGMENT)) {
				...
				List<Node> nodesSubseg = this.myAstContext.currentNode.findNodes(RuleNames.XDI_SUBSEG);
			...
			if (this.ruleName.equals(RuleNames.XDI_SUBSEG)) {
				...
				Node nodeLiteral = this.myAstContext.currentNode.findNode(RuleNames.LITERAL);
				Node nodeXref = this.myAstContext.currentNode.findNode(RuleNames.XDI_XREF);
			...
			if (this.ruleName.equals(RuleNames.XDI_XREF)) {
				...
				this.myAstContext.currentNode.xri = new XDI3XRef(this.myAstContext.currentNode.value, segment, statement, IRI);
			}
