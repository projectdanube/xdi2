...
final public class Rule$HEXDIG extends Rule
{
  private Rule$HEXDIG(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$HEXDIG parse(ParserContext context)
  {
    context.push("HEXDIG");
    ...
    rule = null;
    if (parsed)
      rule = new Rule$HEXDIG(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("HEXDIG", parsed);

    return (Rule$HEXDIG)rule;
  }
}
