...
final public class Rule$IPv6address extends Rule
{
  private Rule$IPv6address(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule$IPv6address parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule$IPv6address(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("IPv6address", parsed);

    return (Rule$IPv6address)rule;
  }
}
