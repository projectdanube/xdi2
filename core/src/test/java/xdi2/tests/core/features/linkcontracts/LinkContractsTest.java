...
public class LinkContractsTest extends TestCase {
	...
	public void testGovernorLinkContract() throws Exception {
		...
		assertNotNull(l1);
		assertEquals(l1.getRequestingAuthority(), XDIAddress.create("+acmebread"));
		assertEquals(l1.getTemplateAuthorityAndId(), XDIAddress.create("+breadunion#registration"));

		GovernorLinkContract l2 = GovernorLinkContract.fromXdiEntity(XdiAbstractEntity.fromContextNode(contextNode));
		assertNotNull(l2);
		assertEquals(l2.getRequestingAuthority(), XDIAddress.create("+acmebread"));
		assertEquals(l2.getTemplateAuthorityAndId(), XDIAddress.create("+breadunion#registration"));
