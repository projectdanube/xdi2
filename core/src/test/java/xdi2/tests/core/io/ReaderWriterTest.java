...
public class ReaderWriterTest extends TestCase {
	...
	private static void assertEqualsGraphs(Graph graph1, Graph graph2) throws Exception {
		...
		while (s1.hasNext()) { XDIStatement s = s1.next().getAddress(); assertTrue(s.toString(), graph2.containsStatement(s)); }
		while (s2.hasNext()) { XDIStatement s = s2.next().getAddress(); assertTrue(s.toString(), graph1.containsStatement(s)); }
