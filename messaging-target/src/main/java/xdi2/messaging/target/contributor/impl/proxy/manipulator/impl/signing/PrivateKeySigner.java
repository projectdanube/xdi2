...
public abstract class PrivateKeySigner extends AbstractSigner implements Signer {
	...
	public Signature<?, ?> sign(Message message) {
		...
		KeyPairSignature signature;

		try {

			String keyAlgorithm = privateKey.getAlgorithm();
			int keyLength = privateKey instanceof RSAKey ? ((RSAKey) privateKey).getModulus().bitLength() : 0;

			signature = (KeyPairSignature) message.createSignature(this.getDigestAlgorithm(), this.getDigestLength(), keyAlgorithm, keyLength, true);
