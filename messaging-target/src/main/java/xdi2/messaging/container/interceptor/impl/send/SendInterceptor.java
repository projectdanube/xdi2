package xdi2.messaging.container.interceptor.impl.send;
...
public class SendInterceptor extends AbstractInterceptor<MessagingContainer> implements OperationInterceptor, Prototype<SendInterceptor> {
	...
	@Override
	public SendInterceptor instanceFor(xdi2.messaging.container.Prototype.PrototypingContext prototypingContext) throws Xdi2MessagingException {
	...
	private void processSendMessage(Message forwardingMessage, SendOperation operation, Graph operationResultGraph, ExecutionContext executionContext) throws Xdi2MessagingException {
		...
		if (forwardingMessage.getToPeerRootXDIArc() != null && forwardingMessage.getToPeerRootXDIArc().equals(messagingContainer.getOwnerPeerRootXDIArc())) {
			...
			if (messagingContainer instanceof AbstractMessagingContainer) {
				...
				LinkContractInterceptor linkContractInterceptor = ((AbstractMessagingContainer) messagingContainer).getInterceptors().getInterceptor(LinkContractInterceptor.class);
		...
		try {
			...
			if ((executionContext.getCurrentMessagingContainer() instanceof GraphMessagingContainer)) {

				GraphMessagingContainer graphMessagingContainer = ((GraphMessagingContainer) executionContext.getCurrentMessagingContainer());
				...
				for (LinkContract pushLinkContract : FullMessagingResponse.getDeferredPushLinkContracts(forwardingMessagingResponse)) {
					...
					CopyUtil.copyContextNode(pushLinkContract.getContextNode(), graphMessagingContainer.getGraph(), null);
					XdiEntityCollection xdiLinkContractIndex = Index.getEntityIndex(graphMessagingContainer.getGraph(), XDILinkContractConstants.XDI_ARC_CONTRACT, true);
				...
				for (LinkContract linkContract : TransportMessagingResponse.getLinkContracts(forwardingMessagingResponse)) {
					...
					CopyUtil.copyContextNode(linkContract.getContextNode(), graphMessagingContainer.getGraph(), null);
					XdiEntityCollection xdiLinkContractIndex = Index.getEntityIndex(graphMessagingContainer.getGraph(), XDILinkContractConstants.XDI_ARC_CONTRACT, true);
